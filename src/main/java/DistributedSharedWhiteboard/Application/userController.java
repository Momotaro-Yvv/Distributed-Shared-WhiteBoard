...
import DistributedSharedWhiteboard.client.JoinWhiteBoard;
...
public class userController {
    ...
    /**
     * This method is called when the user click any "Quit" button or close button of window. If the user
     * is a manager, then a confirmation will be shown for any unsaved changes. Then a message will be sent
     * to the server to indicate this user is quiting.
     *
     * @param event - The event that triggered from clicking the "Quit" button or close button
     */
    @FXML
        ...
        if (event.getEventType() == MouseEvent.MOUSE_PRESSED) {
            ...
            switch (modeID) {
                ...
                case "dm_triangle":
                    ...
                    polygon.getPoints().addAll(Math.min(startX, endX), Math.max(startY, endY),
    ...
    /**
     * Reset whiteboard to initial state which no drawing has been performed.
     */
